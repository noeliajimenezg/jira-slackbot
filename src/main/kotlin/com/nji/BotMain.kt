...
class BotMain : CommandLineRunner {
    ...
    private fun isBetween(startTime: String, endTime: String): Boolean {
        ...
        return (start[Calendar.HOUR_OF_DAY] <= today[Calendar.HOUR_OF_DAY]
                && start[Calendar.MINUTE] <= today[Calendar.MINUTE]
                && today[Calendar.HOUR_OF_DAY] <= end[Calendar.HOUR_OF_DAY]
                && today[Calendar.MINUTE] <= end[Calendar.MINUTE])
    }
}